...
from .profile import ProfileInfo, Profile, ProfileList, DEFAULT_PROFILE
...
class ProfileManager:
    ...
    def __init__(
        ...
        *,  # Force keyword arguments
        ...
        Args:
            profiles_dir: Optional custom directory path for profiles.
                         If None, uses configured default directory.
            env_prefix: Optional prefix for environment variable configuration.
        
        Raises:
            ValueError: If neither profiles_dir nor default config is available.
        """
        if profiles_dir is not None:
            self._dir_path = Path(profiles_dir).expanduser().resolve()
        else:
            env_prefix = env_prefix or ""
            config = ConfigValue(env_prefix)
            dir_name = config.value(ConfigVariable.PROFILES_DIR)
            if not dir_name:
                raise ValueError("No profiles directory specified")
            self._dir_path = Path(dir_name).expanduser().resolve()
                ...
                if info_path.exists():
                    with open(info_path, 'r') as f:
                        ...
                        profiles.append(Profile(
                            id=item.name,
                            info=ProfileInfo.model_validate(data)
                        ))
